...
public class ExceptionsTest extends ParserTest {
    ...
    protected static void testOverflow() {
        ...
        testOverflow((a, b) -> a + b, "+", new Add(VX, VY, new IntComputer()));
        testOverflow((a, b) -> a - b, "-", new Subtract(VX, VY, new IntComputer()));
        testOverflow((a, b) -> a * b, "*", new Multiply(VX, VY, new IntComputer()));
        testOverflow((a, b) -> b == 0 ? Long.MAX_VALUE : a / b, "/", new Divide(VX, VY, new IntComputer()));
        testOverflow((a, b) -> -b, "<- ignore first argument, unary -", new Negate(VY, new IntComputer()));
    }
